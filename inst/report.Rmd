...
df_diff %>% 
  select(Column = variable, 
         !! sym(paste("Type", metadata(df_diff)$names["x"])) := type.x,
         !! sym(paste("Type", metadata(df_diff)$names["y"])) := type.y,
  ...
  as_tibble() %>% 
  {
    ...
    if (type_var %in% names(.x)) {
      type_var_y <- sym(type_var)
      type_var_x <- sym(paste("Type", metadata(df_diff)$names[1]))
      dplyr::mutate(
        .x, 
